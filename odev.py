...
def varlık_ekle():
	...
	def db_ekle():
		...
		if foo:
			...
			data = (code, name, sum_amount, rate, sum_amount * float(live['quotes']['TRY' + code]), kar_zarar, percentage(sum_amount * (1 / güncel_kur), sum_amount))
...
def eskiler():
	...
	def display_changes():
		try:
			...
			print("Gün sayısı geçersiz")
			return
		
		current_date = datetime.datetime.now().date()
		entered_date = current_date - datetime.timedelta(days=date)
		while entered_date <= current_date:
			str_date = str(entered_date)
			if cursor.execute("SELECT * FROM log WHERE date = ?", (str_date,)).fetchone() is []:
		...
		if selection:
			...
			plt.plot(dates[::-1], rates[::-1])  # Reverse the lists to have the oldest date first
			plt.xlabel('Tarih')
			plt.ylabel('Kur')
...
for code in unique_codes:
	...
	yüzde = kar_zarar / (toplam_miktar * güncel_kur) * 100
...
def display_graph():
	...
	# Extract dates and values from the query result
	...
	plt.title('Change in Wealth Over Time')
