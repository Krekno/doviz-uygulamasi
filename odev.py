...
def varlık_ekle():
	...
	def db_ekle():
		...
		if foo:
			...
			data = (code, name, sum_amount, rate, sum_amount * live['quotes']['TRY' + code], kar_zarar, percentage(sum_amount * (1 / güncel_kur), sum_amount))
			...
			data = (code, name, amount, rate, amount * live['quotes']['TRY' + code], 0.0, 0.0)
			data_format = "{:<10} {:<30} {:<15} {:<15} {:<25} {:<20} {:<20}"
			lb_toplam.insert(tk.END, data_format.format(*data))
...
def eskiler():
	...
	lb_eskiler = tk.Listbox(eski_pencere, font=("Courier New", 10), width=64)
...
for code in unique_codes:
	...
	for i in data:
		...
		yüzde.append(percentage(toplam_miktar * (1 / güncel_kur), toplam_miktar))
	lb_toplam.insert(tk.END, data_format.format(code[0], isim, toplam_miktar, güncel_kur, toplam_miktar * güncel_kur, kar_zarar, mean(yüzde) - 100))
